...
The api-gateway service is a Apollo server based graphql endpoint, that needs to have schema and reolvers. At this stage, based on the model documentation, necessary artifacts were added to `api-gateway/schema/order.type.ts` and `api-gateway/resolvers/order.resolvers.ts` . Resolver takes the database repository and service clients via context to serve the client requests .In future any additional resolvers can be placed at `resolvers/`. Resolver has to be a thin layer and adding service layer `api-gateway/modules/order` for handling business logic.
...
The existing scaffolding install the `@assignment/proto` in to `api-gateway` project. The external service calls are placed at `api-gateway/services` for more clarity. The service client generation is done through `api-gateway/services/init.ts` which find deeplinks to the client stubs and generate functions that are callable via JS objects at resolvers. These are also passed to the resolver is done through the `GraphQLContext`.
...
7. External service timeouts are not provided and assumed to be 5 seconds.
8. Exact order events are not specified. Defining event statuses specified at `api-gateway/constant/order-event.constants.ts` Same statuses are used for recording `latestError` field for Orders.
...
1. Backend testing is not fully covering all the functionalities, and lacking scenarios such as mocking external services.
2. Frontend tests are shell structured, and would require more efforts to implement.
3. Some parameters have been hardcoded, such as service timeout, backend url at web-app (could be injected via ), which should be externalised.
